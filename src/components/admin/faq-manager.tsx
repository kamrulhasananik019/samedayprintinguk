...
export default function FaqManager({ faqs, searchTerm, onRefresh, setError, setSuccess, setSaving }: FaqManagerProps) {
  ...
  return (
    <div className="mt-6 grid gap-6 xl:grid-cols-[1.1fr_0.9fr]">
      <form onSubmit={createOrUpdateFaq} className="rounded-3xl border border-[#1b3c53]/10 bg-white p-5 shadow-sm">
        <div className="mb-4 flex items-center justify-between">
          <h3 className="text-lg font-bold text-[#1b3c53]">{editingFaqId ? 'Update FAQ' : 'Create FAQ'}</h3>
          ...
          <div className="grid gap-3 md:grid-cols-2">
            ...
            <label className="flex items-center gap-3 rounded-xl border border-[#d2c1b6] px-3 py-2 text-sm text-[#234c6a]">
              <input
                ...
                className="h-4 w-4 accent-[#1b3c53]"
              />
              Visible on homepage
            </label>
          </div>
        ...
        <div className="mt-4 flex flex-wrap gap-2">
          ...
          <button
            ...
            className="inline-flex items-center gap-2 rounded-xl border border-[#d2c1b6] px-4 py-2 text-sm font-medium text-[#234c6a]"
          >
      ...
      <section className="rounded-3xl border border-[#1b3c53]/10 bg-white p-5 shadow-sm">
        <h3 className="mb-4 text-lg font-bold text-[#1b3c53]">FAQ List ({filteredFaqs.length})</h3>
        <div className="space-y-3">
            ...
            <article key={item.id} className="rounded-2xl border border-[#d2c1b6]/70 bg-[#f4efeb]/50 p-3">
              <div className="flex items-start justify-between gap-3">
                <div>
                  ...
                  <p className="mt-1 text-xs text-[#456882]">Order: {item.sortOrder}</p>
                  <p className="mt-1 text-xs text-[#234c6a]">Status: {item.isActive ? 'Active' : 'Hidden'}</p>
                  <p className="mt-2 text-xs text-[#456882]">{item.answer}</p>
                </div>
              ...
              {busyFaqId === item.id ? <p className="mt-1 text-xs text-[#456882]">Working...</p> : null}
            </article>
          ))}
          {!filteredFaqs.length ? <p className="text-sm text-[#456882]">No FAQs match your search.</p> : null}
        </div>
      </section>
    </div>
  );
